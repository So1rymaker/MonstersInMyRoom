...
public class FirableMonsterGenerator : MonoBehaviour
{
    ...
    IEnumerator MonsterLifecycle(GameObject monster)
    {
        ...
        while (elapsedTime < rushDuration)
        {
            ...
            float speedMultiplier = Mathf.SmoothStep(0f, 5f, progress);

            Vector3 currentPosition = monster.transform.position;

            Vector3 targetPosition = monster.transform.position - shootDirection.normalized * rushDistance;
    ...
    void ShootSphereFromMonster(GameObject monster, Vector3 shootDirection)
    {
        ...
        StartCoroutine(MoveSphere(sphere.transform, targetPosition, 2f));
    }
